...
async def enroll(cfg: SidecarConfig) -> bool:
    """Enroll this device with the FleetGuard Control Center.
    ...
    if not cfg.device_id:
        cfg.device_id = f"fg-dev-{uuid.uuid4().hex[:8]}"

    # Get enrollment token
    if not cfg.enrollment_token:
        print(f"\n🔑 FleetGuard Device Enrollment")
    ...
    try:
        ...
        print(f"   Token saved to {cfg.config_path if hasattr(cfg, 'config_path') else '~/.fleetguard/config.json'}")
        return True

    except httpx.HTTPStatusError as e:
        if e.response.status_code == 400 and "already registered" in str(e.response.text):
            print(f"⚠️  Device already registered. If you have the token, set it in ~/.fleetguard/config.json")
            print(f"   Or delete ~/.fleetguard/config.json and re-enroll.")
            return False
